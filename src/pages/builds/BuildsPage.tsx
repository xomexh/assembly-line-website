...
export function BuildsPage() {
  ...
  useEffect(() => {
    document.body.classList.toggle('builds-theme--onyx', filter === 'Onyx');

    return () => document.body.classList.remove('builds-theme--onyx');
  }, [filter]);
  ...
  return (
    <>
      ...
      />
      <div className={`builds-page builds-page--${filter.toLowerCase()}`}>
        ...
        <section className="catalog section shell">
            ...
            <div className="filter-tabs" role="group" aria-label="Filter build family">
              {(['All', 'Albus', 'Onyx'] as const).map((option) => (
                <button
                  type="button"
                  key={option}
                  data-family={option.toLowerCase()}
          ...
          <div className="catalog__list">
            {filteredBuilds.map((build, index) => {
              ...
              return (
                <article
                  className={`catalog-build catalog-build--${build.family.toLowerCase()}`}
                  id={build.slug}
                  key={build.slug}
                >
